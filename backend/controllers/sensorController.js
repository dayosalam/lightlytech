...
client.on("message", async (topic, message) => {
  ...
  try {
    ...
    const incremental_energy_kwh = power_watts.map(
      (p) => (p * time_elapsed_hours) / 1000
    );
    const incremental_total_energy = incremental_energy_kwh.reduce(
      (a, b) => a + b,
      0
    );
    ...
    let accumulated_bill = parseFloat(
      (incremental_total_energy * rate).toFixed(2)
    );
    ...
    if (lastRecords && lastRecords.length > 0) {
      ...
      if (lastRecord.accumulated_energy !== undefined) {
        accumulated_energy =
          parseFloat(lastRecord.accumulated_energy) + incremental_total_energy;
        accumulated_bill =
          parseFloat(lastRecord.accumulated_bill) +
          parseFloat((incremental_total_energy * rate).toFixed(2));
      }
    }
    ...
    if (error) {
      ...
      console.log(
        `Stored reading for user ${user_id} | Current Energy: ${incremental_total_energy.toFixed(
          4
        )} kWh | Accumulated Energy: ${accumulated_energy.toFixed(
          4
        )} kWh | Bill: ₦${accumulated_bill.toFixed(2)}`
      );
    }
  } catch (err) {
...
const getSensorData = async (req, res) => {
  ...
  try {
    ...
    const total_energy = data.reduce(
      (sum, r) => sum + (r.total_energy || 0),
      0
    );
